...
while Dirs{end,end} == 0
    ...
    if strcmp(RootDir,'/Users/averygreen/Documents/CNSE/Diebold Lab/Data/Samples Hinkle/STM/081016 SB4/m12349_ori_4AveragedIV')
        test = 1;
    end
...
for i = 1:length(CSVFiles)
    ...
    SampleFolder = horzcat('/Users/averygreen/Documents/CNSE/Diebold Lab/Data/Samples Hinkle/STM Matlab Analysis/',SampleID);
    mkdir(SampleFolder)
    ...
    if GoodAvgFit
        ...
        StdHeader{2} = horzcat('Data acquired: ',datestr(Headers{1},'yyddmm HH:MM:SS.FFF'));
        StdHeader{3} = horzcat('nSpectra: ',int2str(nSpectra));
        StdHeader{4} = horzcat('Model: exp(a(1)*(x-a(2)))-exp(a(3)*(x-a(4)))+a(5)*x+a(6)');
        
        ExportIDTime = horzcat(SampleFolder,'/',datestr(Headers{1},'yy-mm-dd_HH-MM-SS-FFF'));
...
fprintf(File,'%s',StdHeader{2}(end-14:end));
